...
export function ensureConversationItem(msg) {
    ...
    const html = `
        <div data-conversation="${msg.sender.toLowerCase()} ${msg.body.toLowerCase()}" data-name="${escaped(msg.sender)}" data-avatar="${msg.sender_avatar}" data-has-avatar="${msg.sender_has_avatar ? '1' : '0'}" data-status="offline" data-about="${escaped(msg.sender_bio)}" data-real-name="${escaped(msg.sender)}" data-username="${msg.sender_username}" data-custom-name="${msg.custom ? escaped(msg.sender) : ''}" onclick="openConversation(this.dataset.name, this.dataset.avatar, this.dataset.status, this.dataset.about, this.dataset.customName, this.dataset.realName, this.dataset.username, this.dataset.hasAvatar === '1')" class="flex items-center gap-2.5 px-3 py-2.5 cursor-pointer transition-all duration-150 hover:bg-white/5">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between">
                    <p class="flex items-center gap-1.5 min-w-0 text-xs font-medium truncate text-white/80">
                        <span class="truncate">${escaped(msg.sender)}</span>
