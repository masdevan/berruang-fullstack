@props(['name', 'avatar', 'lastMessage' => '', 'time' => '', 'unread' => 0, 'online' => false, 'active' => false, 'about' => ''])

<div data-conversation="{{ strtolower($name) }} {{ strtolower($lastMessage) }}" data-name="{{ $name }}" data-avatar="{{ $avatar }}" data-online="{{ $online ? '1' : '0' }}" data-about="{{ $about }}" onclick="openConversation(this.dataset.name, this.dataset.avatar, this.dataset.online === '1', this.dataset.about)" class="flex items-center gap-2.5 px-3 py-2.5 cursor-pointer transition-all duration-150 hover:bg-white/5 {{ $active ? 'bg-white/5' : '' }}">
