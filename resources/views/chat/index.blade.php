...
@section('content')
    ...
    <div id="sidebar-right" class="shrink-0 overflow-hidden fixed inset-y-0 right-0 z-40 w-full translate-x-full transition-transform duration-200 md:static md:translate-x-0 md:transition-none md:w-0">
    ...
    <div id="media-modal" class="hidden fixed inset-0 z-50 bg-black/30 backdrop-blur-xl items-center justify-center p-4" onclick="closeMediaModal()">
        <button type="button" onclick="closeMediaModal()"
                class="absolute top-[calc(1rem+env(safe-area-inset-top))] right-[calc(1rem+env(safe-area-inset-right))] w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-colors cursor-pointer">
        ...
        <div id="media-modal-content" class="w-full max-w-5xl flex items-center justify-center" onclick="event.stopPropagation()">
            <img id="media-modal-image" src="" alt="Media preview" class="hidden w-full max-h-[90dvh] object-contain rounded-lg">
            <video id="media-modal-video" src="" controls class="hidden w-full max-h-[90dvh] rounded-lg bg-black"></video>
        </div>
    </div>
